...
class RecordControllerSpec extends IntegrationSpec {
    ...
    def "get should return a 401 (SC_UNAUTHORIZED) if there is no userId when requesting an embargoed record"() {
        ...
        new Record(projectActivityId: createProjectActivity("pa1", "project1", future.getTime()), occurrenceID: "1234", userId: "id1", projectId: "project1", status: ACTIVE, isDataManagementPolicyDocumented: false, dataAccessMethod: "dataAccessMethod", dataQualityAssuranceMethods: ["dataownercurated"], "nonTaxonomicAccuracy": "low", "temporalAccuracy": "low", "speciesIdentification": "low", methodType     : "opportunistic","spatialAccuracy": "low").save(flush: true, failOnError: true)
    ...
    private static String createProjectActivity(String id, String projectId, Date embargoDate) {
        ProjectActivity pa = new ProjectActivity(projectActivityId: id,
                ...
                startDate: new Date(),
                status: ACTIVE,
                isDataManagementPolicyDocumented: false, dataAccessMethod: "dataAccessMethod", dataQualityAssuranceMethods: ["dataownercurated"], "nonTaxonomicAccuracy": "low", "temporalAccuracy": "low", "speciesIdentification": "low", methodType     : "opportunistic","spatialAccuracy": "low",
