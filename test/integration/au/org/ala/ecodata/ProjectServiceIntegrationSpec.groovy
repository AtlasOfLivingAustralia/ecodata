...
class ProjectServiceIntegrationSpec extends IntegrationSpec {
    ...
    private static createHierarchy() {
        ...
        ProjectActivity projectActivity1 = new ProjectActivity(projectActivityId: "proAct1", projectId: project.projectId, description: "d", name: "n", startDate: new Date(), isDataManagementPolicyDocumented: false, dataAccessMethods: ["na"], dataQualityAssuranceMethods: ["dataownercurated"], "nonTaxonomicAccuracy": "low", "temporalAccuracy": "low", "speciesIdentification": "low", "spatialAccuracy": "low", methodType     : "opportunistic", methodName: "Opportunistic/ad-hoc observation recording", dataSharingLicense: "CC BY", status: ACTIVE).save(failOnError: true, flush: true)
        ProjectActivity projectActivity2 = new ProjectActivity(projectActivityId: "proAct2", projectId: project.projectId, description: "d", name: "n", startDate: new Date(), isDataManagementPolicyDocumented: false, dataAccessMethods: ["na"], dataQualityAssuranceMethods: ["dataownercurated"], "nonTaxonomicAccuracy": "low", "temporalAccuracy": "low", "speciesIdentification": "low", "spatialAccuracy": "low", methodType     : "opportunistic", methodName: "Opportunistic/ad-hoc observation recording", dataSharingLicense: "CC BY", status: ACTIVE).save(failOnError: true, flush: true)
