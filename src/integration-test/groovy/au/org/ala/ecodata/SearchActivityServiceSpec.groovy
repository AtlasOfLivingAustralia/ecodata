 ...
 */
@Integration
class SearchActivityServiceSpec extends Specification {
     ...
     //   mockDomain Activity
      ...
      //  Activity.withNewTransaction {
       ...
       // }
        ...
        activityService.lockService = Stub(LockService)
        activityService.outputService = outputService

        CommonService commonService = new CommonService()
        commonService.grailsApplication = grailsApplication //Stub(GrailsApplication)
        activityService.commonService = commonService
    }
    ...
    @Unroll
    def "activities can be searched for without supplying dates - failed ones"(criteria, expectedActivityIds) {

        when:
        def results
        Activity.withNewTransaction {
            results = activityService.search(criteria, LevelOfDetail.NO_OUTPUTS.name())
    ...
    def "activities can be searched by start date #startDate"(startDate, planned, criteria, expectedActivityIds) {
        ...
        Activity.withNewTransaction { // There is probably a neater way to do this but the MongoDbTestMixin doesn't work with the ServiceUnitTestMixin
            ...
            results = activityService.search(criteria, df.parse(startDate), null, dateProperty, LevelOfDetail.NO_OUTPUTS.name())
            results.sort { a1, a2 -> a1.activityId <=> a2.activityId }
        }
        then:
        results.collect {it.activityId} == expectedActivityIds
    ...
    def "activities can be searched by end date"(endDate, planned, criteria, expectedActivityIds) {
        ...
        Activity.withNewTransaction { // There is probably a neater way to do this but the MongoDbTestMixin doesn't work with the ServiceUnitTestMixin
            ...
            DateFormat df = new SimpleDateFormat('yyyy/MM/dd')
            results = activityService.search(criteria, null, df.parse(endDate), dateProperty, LevelOfDetail.NO_OUTPUTS.name())
            results.sort { a1, a2 -> a1.activityId <=> a2.activityId }
        }
        then:
        results.collect {it.activityId} == expectedActivityIds
 ...
 /*   def "when an activity is cancelled or deferred, existing Output data should be deleted"(String progressToAssign, boolean shouldBeDeleted) {
        ...
        Activity.withTransaction {
            activityService.update([activityId: id, progress: progressToAssign], id)
        }
    ...
    def "when an activity type is changed, existing Output data should be deleted"() {
        ...
        when:
        Activity.withTransaction {
            activityService.update([activityId: id, type: 'Type 2'], id)
        }
