...
const imageCarousel = '<img style="height:300px" src="https://dl.dropboxusercontent.com/s/8nct1vmhq86gpf3/plate.png?dl=0" alt="Plate">' +
...
const Output_CoralWatch_coralObservationsRow = function (data, dataModel, context, config) {
    const self = this;
    ...
    self.colourCodeLightest = ko.observable().extend({
        metadata: {
            metadata: self.dataModel['colourCodeLightest'],
            context: self.$context,
            config: config
        }
    });
    self.colourCodeDarkest = ko.observable().extend({
        metadata: {
            metadata: self.dataModel['colourCodeDarkest'],
            context: self.$context,
            config: config
        }
    });
    self.colourCodeAverage = ko.observable().extend({numericString: 2});
    self.typeOfCoral = ko.observable().extend({
        metadata: {
            metadata: self.dataModel['typeOfCoral'],
            context: self.$context,
            config: config
        }
    });
    const coralSpeciesConfig = _.extend(config, {
        printable: '',
        dataFieldName: 'coralSpecies',
        output: 'CoralWatch',
        surveyName: ''
    });
    ...
    self.colourCodeLightest.subscribe(function (obj) {
        const lightCode = self.colourCodeLightest();
        if (lightCode) {
            const codes = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'D1', 'D2', 'D3', 'D4', 'D5', 'D6', 'E1', 'E2', 'E3', 'E4', 'E5', 'E6'];
            const lightCodeIndex = codes.indexOf(lightCode);
            const msg = [];
            $.each(codes, function (key, value) {
                const codeNumber = lightCode.charAt(1);
                const integerVal = parseInt(value.charAt(1));
            ...
            bootbox.prompt({
                ...
                callback: function (result) {
                    ...
                    if (self.colourCodeDarkest() && self.colourCodeLightest()) {
                        const total = parseInt(self.colourCodeDarkest().charAt(1)) + parseInt(self.colourCodeLightest().charAt(1));
                        if (total > 0) {
    ...
    self.typeOfCoral.subscribe(function (obj) {
        ...
        let msg = "<h2>Coral image not available!</h2>";
        if (url) {
            msg = "<img src='" + url + "'></img>";
        }
    });
...
// This is re-declaration of context when this script is included on the full page.
...
self.data.depthInMetres.subscribe(function (obj) {
    let feet = (parseFloat(self.data.depthInMetres()) * 3.28084);
    feet = feet.toFixed(2);
    self.data.depthInFeet(feet);
});

self.data.waterTemperatureInDegreesCelcius.subscribe(function (obj) {
    let farenheit = parseFloat((parseFloat(self.data.waterTemperatureInDegreesCelcius())).toFixed(2) * (9 / 5) + 32);
...
self.transients.convert = ko.computed(function () {
    const tempF = (self.data.waterTemperatureInDegreesFarenheit() || '0').toString();
    const tempC = (self.data.waterTemperatureInDegreesCelcius() || '0').toString();
    if (tempF === '0' && tempC === '0') {
        // Don't update
    } else {
        let tempCFloat = (parseFloat(tempF).toFixed(2) - 32) * (5 / 19);
        tempCFloat = tempCFloat.toFixed(2);
        self.data.waterTemperatureInDegreesCelcius(tempCFloat);
    }

    // update depth in metres from depth in feet
    let meters = (parseFloat(self.data.depthInFeet()).toFixed(2) / 3.28084);
