        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (record["Project IDs"] && record['Matching name']) {
                ...
                givenIDs.forEach(function (id) {
                    var projects = db.project.find({$and:[{externalId: id},{status:{$ne:'deleted'}}]},{projectId:1,externalId:1,organisationName:1,organisationId:1,status:1})
                    ...
                    if (count == 1) {
                        ...
                        //Try grant ID
                        var projects = db.project.find({$and:[{grantId: id},{status:{$ne:'deleted'}}]},{projectId:1,externalId:1,organisationName:1,organisationId:1,status:1})
